...
✅ **ปิดสวิตช์แล้วเมื่อ 2026-08-23** หลังแอป iOS เปลี่ยนไปใช้ `GET /pets/:id/avatar`
(`vertex-app` commit `6f3572b` — build ลงเครื่องจริงและทดสอบแล้ว)

**วัดจาก traffic จริงของแอปบน production** (ผู้ใช้มีแมว 3 ตัว)

| | ขนาดที่โหลดต่อการเปิดหนึ่งครั้ง |
|---|---|
| ก่อนแก้ | **9.73 MB** |
| หลังแก้ เปิดครั้งแรก | 3.65 MB |
| หลังแก้ เปิดครั้งต่อไป | **4.05 KB** |

ครั้งต่อไปเล็กลง **2,461 เท่า** เพราะ avatar ทั้ง 3 รูปได้ `304` ที่ body 0 ไบต์

หมายเหตุ: แอปเรียก `GET /pets` สองครั้งซ้อนต่อการเปิดหนึ่งครั้ง (พฤติกรรมเดิมของแอป)
แต่ avatar ถูกดึงครั้งเดียวต่อแมวหนึ่งตัว เพราะ `PetAvatarStore` รวม request
ที่ซ้อนกันเป็นตัวเดียว — ถ้าไม่มีตัวนี้จะกลายเป็น 6 request

แอปเก็บรูปกับ ETag ไว้ในเครื่อง แล้วส่ง `If-None-Match` รอบถัดไป
ได้ `304` ที่ไม่มี body เลย และรองรับเซิร์ฟเวอร์ทั้งสองแบบพร้อมกัน
จึงสลับสวิตช์ไปมาได้ทันทีโดยไม่ต้อง build แอปใหม่
...
| gate | สถานะ |
|---|---|
| `go vet` (รวม tag integration) | ✅ |
| `golangci-lint` | ✅ |
| `go test -race` | ✅ |
| **integration test** | ✅ เพิ่มเข้า `deploy.yml` แล้ว |
| `govulncheck` | ✅ |
| `flyway migrate` + `FAIL_ON_MISSING_LOCATIONS` | ✅ |
| **Trivy scan บน image** | ✅ CRITICAL/HIGH ที่มีทางแก้แล้ว |
| **coverage gate** | ✅ เพดานล่าง 70% |

🔴 **ช่องที่ปิดไปแล้ว — สำคัญที่สุดของเฟสนี้**

`ci.yml` ตั้ง `branches-ignore: [main]` ไว้ แต่เราทำงานบน main โดยตรง
แปลว่า **integration test ไม่เคยรันบน CI เลย**สำหรับงานที่ push ขึ้น main
ซึ่งเป็นเส้นทางเดียวที่ deploy ขึ้น production จริง
ที่ผ่านมารอดเพราะรันในเครื่องก่อน push ทุกครั้ง — ไม่ใช่เพราะระบบป้องกันไว้

ตอนนี้ `deploy.yml` มี postgres + Flyway + integration test ครบ
ต้องผ่านทั้งหมดก่อนถึงจะ build image
...
`repository`/`application` ซึ่ง Go ไม่นับให้ถ้าไม่ระบุ

**Authorization matrix**

`authz_matrix_integration_test.go` ครอบ **39 ช่อง** ของ
(ผู้เรียก 4 แบบ × endpoint 8 ตัว) บนฐานข้อมูลจริง

| | อ่านข้อมูล | บันทึกน้ำ | บันทึก litter | แก้โปรไฟล์ | ลบสัตว์เลี้ยง | ดูผู้ดูแล |
|---|---|---|---|---|---|---|
| เจ้าของ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| ผู้ดูแล (มีสิทธิ์ครบ) | ✓ | ✓ | ✓ | ✓ | 403 | 403 |
| ผู้ดูแล (เฉพาะ MANAGE_WATER) | ✓ | ✓ | 403 | 403 | 403 | 403 |
| คนนอก | 404 | 404 | 404 | 404 | 404 | 404 |

คนนอกได้ **404 ไม่ใช่ 403** โดยตั้งใจ — 403 เท่ากับยืนยันว่าสัตว์เลี้ยงตัวนั้น
มีอยู่จริง ซึ่งทำให้ไล่เดา UUID เพื่อสำรวจว่าในระบบมีอะไรบ้างได้
เทสต์ตรวจด้วยว่า error response ของคนนอกไม่มีข้อมูลสัตว์เลี้ยงหลุดไป

**บั๊กที่ตารางนี้จับได้ทันทีที่เขียนเสร็จ**

ส่ง `species` ที่ไม่มีใน master data แล้วได้ **500** แทนที่จะเป็น 400
เพราะ FK violation ตกไปที่ `default: Internal(...)` ผู้เรียกจึงไล่สาเหตุไม่ได้
...
## 9.5 งานที่ค้างอยู่ (2026-08-23)

รายการเต็มอยู่ที่ `vertex-backend/infra/BACKLOG.md`

⚠️ ไฟล์นั้นอยู่ใน `vertex-backend/` ซึ่ง**ไม่ใช่ git repo** — สรุปย่อไว้ที่นี่
เพราะไฟล์นี้อยู่ใน git จริง ถ้าเครื่องหายจะยังเหลือรายการนี้ไว้

| ระดับ | เรื่อง | ติดที่ |
|---|---|---|
| 🔴 | **สำรองข้อมูลอัตโนมัติ** | รอตัดสินใจว่าจะส่ง backup ไปเก็บที่ไหน |
| 🟠 | รหัสผ่าน superuser ของ PostgreSQL อ่อน | ทำได้ทันที ไม่ต้อง restart |
| 🟠 | private key เก่ายังดึงได้จาก GitHub ผ่าน SHA เดิม | ต้องลบ repo แล้วสร้างใหม่ |
| 🟡 | เอา `public` ออกจาก `search_path` ของ pet/auth | ทำรวมกับ deploy รอบหน้า |
| 🟡 | รูปสัตว์เลี้ยงใหญ่เกินจำเป็น (ใหญ่สุด 2 MB) | แก้ที่แอป ย่อก่อนอัปโหลด |
| 🟢 | schema snapshot ของ `auth` / `event` | ใช้สคริปต์เดิม |
| 🟢 | Phase 5 ข้อ 4, 7, 8, 10 | ข้อ 8 ต้องตัดสินใจกับ PO ก่อน |

**ข้อ 🔴 สำคัญที่สุด** — PVC ใช้ `local-path` ผูกข้อมูลไว้กับ node เดียว
คลัสเตอร์มี node เดียว และไม่มีสำเนาที่อื่นเลย
node เสียหรือ disk พัง = ข้อมูลหายถาวร
ตอนนี้มีแต่ backup ที่ทำด้วยมือเก็บในเครื่องผู้ใช้

MinIO ในคลัสเตอร์เดียวกันช่วยเรื่อง "เผลอลบ database" แต่**ไม่ช่วยเรื่อง
node เสีย** เพราะยังอยู่ดิสก์ก้อนเดียวกัน — ต้องเป็นที่ที่อยู่นอกเครื่องจริง
